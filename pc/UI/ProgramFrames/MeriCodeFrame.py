...
class MeriCodeFrame(customtkinter.CTkFrame):
    def __init__(self, parent, frameParent, *args, **kwargs):
        ...
        self.grid(row=2, column=0, padx=(5, 5), pady=(5, 5), sticky=tkinter.NW)
        ...
        self.slicingSwitch = customtkinter.CTkSwitch(master=self, text="", command= lambda: self.Switched(), progress_color=Colors.BUTTON, button_color=Colors.BUTTON)
        ...
        self.sendTestFile = customtkinter.CTkButton(master=self, text="Send MeriCode",  fg_color=Colors.BUTTON, hover_color=Colors.BUTTONHOVER, font=("", 11), width=200, height=28, text_color=Colors.BUTTONTEXT, command= lambda: self.StartSendingFile())
        self.sendTestFile.grid(row=2, column=0, columnspan=6, sticky=tkinter.SW)
        ...
        self.showButton = customtkinter.CTkButton(master=self, text="Show MeriCode",  fg_color=Colors.BUTTON, hover_color=Colors.BUTTONHOVER, font=("", 11), width=125, height=25, text_color=Colors.BUTTONTEXT, command= lambda: self.parent.hardwareAcceleratedCanvas.canvas.ShowMeriCode(self.cutting))
        self.showButton.grid(row=3, column=0, columnspan=2, padx=(30, 5), sticky=tkinter.W)

        self.showButton = customtkinter.CTkButton(master=self, text=">",  fg_color=Colors.BUTTON, hover_color=Colors.BUTTONHOVER, font=("", 11), width=25, height=25, text_color=Colors.BUTTONTEXT, command= lambda: self.ShowNextLine(1))
        self.showButton.grid(row=3, column=2, sticky=tkinter.W)
    ...
    def GenerateMeriCode(self):
        self.parent.hardwareAcceleratedCanvas.canvas.CanvasToMeriCode()
        canvasToMericode = self.parent.hardwareAcceleratedCanvas.canvas.canvasToMericode
